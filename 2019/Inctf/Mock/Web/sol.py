...
key=[1847,1878,443,384]


source = "ТЄЂђЭБёщФКєѹЯћЅѹѺеЂЖХЌДГѸеДЙѴѕМ"

crypt_code=''
index=0
for i in source:
    crypt_code+=chr(ord(i)^(key[index%4]))
    index+=1
...
for hashind in range(4):
    print("-----------------")
    key=[]
    for p in possibleset:     #Iterate through Strings
        ...
        for j in range(hashind + 4, len(the_string), 4):     #Iterate Through Particular Index Of the The String
            if chr(testsol ^ ord(the_string[j])) not in possibleset:  #If it not Present in the Printable Ascii Range Exit from the loop
                break
        else:
            print(testsol)
            key.append(testsol)
